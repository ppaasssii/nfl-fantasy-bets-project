...
export interface AvailableBetDetail {
    ...
    bet_subject_name?: string | null;
    bet_subject_team_id?: string | null;
    bet_subject_team_affiliation?: 'home' | 'away' | null;
}

// --- HELPER FUNCTIONS (Remain the same) ---
const groupOddsByBetTypeName = (oddsList: AvailableBetDetail[]): Record<string, AvailableBetDetail[]> => oddsList.reduce((acc, odd) => {
...
const extractPlayerNameFromSelection = (selectionName: string, betTypeName: string, fallbackPlayerName?: string | null): string => {
    ...
    let name = selectionName.replace(/ (Over|Under) [\d.\-]+$/, '').replace(/ (Yes|No)$/i, '').trim();
    const betTypeBase = betTypeName.replace(/^Player /i, '').replace(/ O\/U$/i, '').replace(/ Yes\/No$/i, '').trim();
    if (name.toLowerCase().endsWith(betTypeBase.toLowerCase()) && betTypeBase.length > 0) {
        name = name.substring(0, name.length - betTypeBase.length).trim();
    } else if (name.toLowerCase().startsWith(betTypeBase.toLowerCase()) && betTypeBase.length > 0) {
    ...
    if (name === selectionName && (betTypeName.toLowerCase().includes("o/u") || betTypeName.toLowerCase().includes("yes/no"))) {
        ...
        if (parts.length > 2) {
            if (parts[0] && parts[1] && (parts[0].length > 1 || parts[1].length > 1)) {
                name = `${parts[0]} ${parts[1]}`;
                if (parts.length > 3 && parts[2] && (parts[2].length > 1 && !/^[A-Z]+$/.test(parts[2]) && !['Jr', 'Sr', 'II', 'III', 'IV'].includes(parts[2]))) {
            ...
            let potentialPlayerName = selectionName;
            commonPropWords.forEach(word => {
                potentialPlayerName = potentialPlayerName.replace(new RegExp(` ${word}(\\s.*)?$`, 'i'), '').trim();
            });
            name = potentialPlayerName === selectionName ? "Unknown Player" : potentialPlayerName;
        }
    }
    return name.trim() || "Unknown Player";
};
...
const processOddsForMarketDisplay = (oddsListForSpecificMarket: AvailableBetDetail[], originalBetTypeName: string): ProcessedMarket => {
    ...
    oddsListForSpecificMarket.forEach(odd => {
        ...
        if (!linesMap.has(lineKey)) {
            linesMap.set(lineKey, {lineValue: odd.line ?? null, options: []});
        }
...
const GameDetailPage: React.FC = () => {
    ...
    const fetchGameDetailsAndOdds = useCallback(async () => { /* ... same fetching logic ... */
        ...
        try {
            const {
                data: gameDetails,
                error: gameError
            } = await supabase.from('games').select('*').eq('id', gameIdNumber).single();
            ...
            const {
                data: fetchedOdds,
                error: oddsError
            } = await supabase.from('available_bets').select(`*, bet_type:bet_types!inner(id, name, api_market_key, description), bet_subject_name, bet_subject_team_id, bet_subject_team_affiliation`).eq('game_id', gameDetails.id).eq('is_active', true).order('bet_type_id').order('line', {
                ascending: true,
                nullsFirst: true
            }).order('selection_name');
            if (oddsError) throw oddsError;
            const oddsWithCtx = fetchedOdds?.map(o => ({
                ...o,
                game_info_for_slip: {
                    id: gameDetails.id,
                    api_game_id: gameDetails.api_game_id,
            ...
            console.error('Err GameDetailFetch:', err);
            setError(err.message);
            toast.error(`Load err: ${err.message}`);
        } finally {
            setLoading(false);
        }
    }, [dbGameId]);
    useEffect(() => {
        fetchGameDetailsAndOdds();
    }, [fetchGameDetailsAndOdds]);

    const handleOddClick = (odd: AvailableBetDetail) => { /* ... same toggle logic ... */
        if (!odd.is_active && !isOddInBetSlip(odd.id)) {
            toast.warn('This betting option is no longer active.');
            return;
        }
        if (isOddInBetSlip(odd.id)) {
            removeFromBetSlip(odd.id);
        } else {
            addToBetSlip(odd, false);
        }
    };
    ...
    const processedTabsData = useMemo(() => { /* ... same processing logic ... */
        ...
        oddsData.forEach(odd => {
            ...
            if (marketKey.startsWith('player_')) dataByTab['Player Bets'].push(odd); else if (!periodKeysPattern.test(marketKey)) dataByTab['Game Bets'].push(odd);
        });
        ...
        if (dataByTab['Game Bets']?.length > 0) {
            const gmgbt = groupOddsByBetTypeName(dataByTab['Game Bets']);
            const pGML: ProcessedMarket[] = [];
            for (const btn in gmgbt) pGML.push(processOddsForMarketDisplay(gmgbt[btn], btn));
            contentByTab['Game Bets'] = pGML;
        }
        if (dataByTab['Player Bets']?.length > 0) {
            const ppat: {
                home: AvailableBetDetail[],
                away: AvailableBetDetail[],
                unknown: AvailableBetDetail[]
            } = {home: [], away: [], unknown: []};
            dataByTab['Player Bets'].forEach(odd => {
                if (odd.bet_subject_team_affiliation === 'home') ppat.home.push(odd); else if (odd.bet_subject_team_affiliation === 'away') ppat.away.push(odd); else ppat.unknown.push(odd);
            });
            contentByTab['Player Bets'] = {homePlayers: {}, awayPlayers: {}, unknownPlayers: {}};
            const processPg = (pOds: AvailableBetDetail[]) => {
                const gbn: Record<string,Record<string,AvailableBetDetail[]
                >>= {};
                pOds.forEach(o => {
                    const btn = o.bet_type?.name || "UnkProp";
                    const pN = o.bet_subject_name || extractPlayerNameFromSelection(o.selection_name, btn, o.bet_subject_name);
                    if (!gbn[pN]) gbn[pN] = {};
                    if (!gbn[pN][btn]) gbn[pN][btn] = [];
                    gbn[pN][btn].push(o);
                });
                const fpPd: Record<string, ProcessedMarket[]> = {};
                for (const pN in gbn) {
                    const pM: ProcessedMarket[] = [];
                    for (const btn in gbn[pN]) pM.push(processOddsForMarketDisplay(gbn[pN][btn], btn));
                    fpPd[pN] = pM;
                }
                return fpPd;
            };
            ...
            if (ppat.unknown.length > 0 && Object.keys(ppat.unknown).length > 0) contentByTab['Player Bets'].unknownPlayers = processPg(ppat.unknown);
        }
    ...
    const renderOddButton = (odd: AvailableBetDetail | undefined, label?: string) => { /* ... same button styling and logic from previous, uses sleeper theme classes ... */
        ...
        return (<button key={odd.id + (label || '') + '-detail-odd-btn'} onClick={() => handleOddClick(odd)}
                        disabled={!odd.is_active && !isSelected}
                        className={`w-full p-3 text-left rounded-md focus:outline-none flex flex-col justify-center items-center group shadow-md h-full transition-all duration-150 ease-in-out ${isSelected ? 'bg-sleeper-accent text-sleeper-text-on-accent ring-1 ring-white/50' : 'bg-sleeper-surface-200 hover:bg-sleeper-primary focus:ring-2 focus:ring-sleeper-primary'} ${!odd.is_active && !isSelected ? 'bg-sleeper-surface-100 text-gray-500 cursor-not-allowed opacity-60' : ''}`}>
            <span
                className={`text-xs font-medium mb-0.5 ${isSelected ? 'text-sleeper-text-on-accent' : 'text-sleeper-text-secondary group-hover:text-sleeper-text-on-primary'}`}>{label || odd.selection_name}</span><span
            className={`text-md font-bold ${isSelected ? 'text-sleeper-text-on-accent' : 'text-sleeper-interactive group-hover:text-sleeper-text-on-primary'}`}>{decimalOddForDisplay.toFixed(2)}</span>
        </button>);
    };

    if (loading) return <div className="text-center py-10 text-xl text-sleeper-accent animate-pulse">Loading Game
        Details...</div>;
    if (error) return <div
        className="flex flex-col items-center justify-center text-center py-10 bg-sleeper-surface-100 rounded-lg shadow-md border border-sleeper-error">
        <ExclamationTriangleIcon className="w-12 h-12 text-sleeper-error mb-3"/><h3
        className="text-xl text-sleeper-error mb-2">Error Loading Game Data</h3><p
        className="text-sleeper-text-secondary">{error}</p></div>;
    if (!game) return <div
        className="flex flex-col items-center justify-center text-center py-10 bg-sleeper-surface-100 rounded-lg shadow-md border border-sleeper-border">
        <CalendarDaysIcon className="w-12 h-12 text-sleeper-text-secondary mb-3 opacity-50"/><h3
        className="text-xl text-sleeper-text-primary">Game Not Found</h3><p className="text-sleeper-text-secondary">The
        requested game could not be found.</p></div>;
    ...
    const formattedDate = `${gameDate.toLocaleDateString(undefined, {
        ...
        month: 'short',
        day: 'numeric'
    })} - ${gameDate.toLocaleTimeString(undefined, {hour: '2-digit', minute: '2-digit', hour12: true})}`;
    ...
    return (
        <div className="container mx-auto px-2 sm:px-4 py-6"> {/* text-sleeper-text-primary applied by body */}
            <Link to="/"
                  className="inline-flex items-center mb-6 text-sm text-sleeper-interactive hover:text-sleeper-primary transition-colors font-medium"><ArrowLeftIcon
                className="h-5 w-5 mr-1.5"/> Back to Games</Link>

            {/* Game Header */}
            <div className="bg-sleeper-surface-100 p-4 sm:p-6 rounded-xl shadow-xl mb-8 border border-sleeper-border">
                <div className="flex flex-col sm:flex-row justify-between items-center mb-2">
                    <h1 className="text-2xl sm:text-3xl font-bold text-sleeper-text-primary truncate text-center sm:text-left">{game.away_team}
                        <span className="text-sleeper-text-secondary mx-1 text-xl">@</span> {game.home_team}</h1>
                    <div className="text-sm text-sleeper-text-secondary mt-2 sm:mt-0 text-center sm:text-right">
                        <div>{formattedDate}</div>
                        <div className="capitalize mt-0.5">{isLive ? <span
                            className="px-2 py-0.5 text-xs font-bold text-white bg-red-600 rounded-full animate-pulse">LIVE</span> : game.status}</div>
                    </div>
                </div>
                {isFinal && game.home_score != null && game.away_score != null && (
                    <p className="text-xl text-center sm:text-right font-semibold text-sleeper-primary mt-2">Final
                        Score: {game.away_team} <span
                            className="text-sleeper-accent">{game.away_score}</span> - {game.home_team} <span
                            className="text-sleeper-accent">{game.home_score}</span></p>
                )}
            </div>

            {(!oddsData || processedTabsData.tabOrder.length === 0) && !loading ? (
                <div
                    className="text-center text-lg text-sleeper-text-secondary py-10 bg-sleeper-surface-100 rounded-lg shadow-md border border-sleeper-border">No
                    active odds available for this game at the moment.</div>
            ) : (
                <Tab.Group>
                    <Tab.List
                        className="flex space-x-1 rounded-xl bg-sleeper-surface-100 p-1.5 mb-6 shadow-md border border-sleeper-border overflow-x-auto custom-scrollbar">
                        {processedTabsData.tabOrder.map((tabName) => (
                            <Tab key={tabName} as={Fragment}>
                                {({selected}) => (<button
                                    className={`flex-auto whitespace-nowrap rounded-lg py-2.5 px-3 sm:px-5 text-sm font-semibold leading-5 focus:outline-none transition-all duration-150 ease-in-out ${selected ? 'bg-sleeper-primary text-sleeper-text-on-primary shadow-lg' : 'text-sleeper-text-secondary hover:bg-sleeper-surface-200 hover:text-sleeper-text-primary'}`}>{tabName}</button>)}
                            </Tab>
                        ))}
                    </Tab.List>
                    <Tab.Panels className="mt-2">
                        {processedTabsData.tabOrder.map((tabName) => (
                            <Tab.Panel key={tabName}
                                       className="focus:outline-none space-y-6"> {/* Removed rounded-xl bg-transparent */}
                                ...
                                {tabName === 'Player Bets' && ['homePlayers', 'awayPlayers', 'unknownPlayers'].map(playerGroupKey => {
                                    ...
                                    return (<Disclosure key={playerGroupKey} as="div"
                                                        className="bg-sleeper-surface-100 rounded-lg shadow-lg border border-sleeper-border"
                                                        defaultOpen={true}>
                                        {({open}) => (<>
                                            <Disclosure.Button
                                                className={`flex justify-between w-full px-4 py-3 text-lg font-semibold text-left text-sleeper-text-primary hover:bg-sleeper-surface-200/70 ${open ? 'rounded-t-lg' : 'rounded-lg'}`}>
                                                <span
                                                    className="flex items-center">{React.createElement(icon, {className: "h-6 w-6 mr-3 text-sleeper-primary"})} {groupTitle}</span>
                                                <ChevronUpIcon
                                                    className={`${open ? 'rotate-180' : ''} w-6 h-6 text-sleeper-primary`}/>
                                            </Disclosure.Button>
                                            <Disclosure.Panel
                                                className="px-3 py-3 text-sm border-t border-sleeper-border space-y-4">
                                                {Object.entries(playersToRender).map(([playerName, marketsForPlayer]) => (
                                                    <Disclosure key={`${playerName}-${playerGroupKey}`} as="div"
                                                                className="bg-sleeper-surface-200 rounded-md shadow-md"
                                                                defaultOpen={false}>
                                                        {({open: playerOpen}) => (<>
                                                            <Disclosure.Button
                                                                className={`flex justify-between w-full px-4 py-2.5 text-md font-medium text-left text-sleeper-text-primary hover:bg-sleeper-border/20 ${playerOpen ? 'rounded-t-md' : 'rounded-md'}`}>
                                                                <span className="flex items-center"><UserCircleIcon
                                                                    className="h-5 w-5 mr-2 text-sleeper-interactive"/>{playerName}</span>
                                                                <ChevronUpIcon
                                                                    className={`${playerOpen ? 'rotate-180' : ''} w-5 h-5 text-sleeper-interactive`}/>
                                                            </Disclosure.Button>
                                                            <Disclosure.Panel
                                                                className="px-3 pt-3 pb-3 text-xs border-t border-sleeper-border/50 space-y-3">
                                                                {marketsForPlayer.map(market => (<div
                                                                    key={market.fullBetTypeName + '-' + market.marketName}
                                                                    className="py-1.5">
                                                                    <h5 className="text-sm font-semibold text-sleeper-text-secondary mb-2">{market.marketName}</h5>
                                                                    {market.lines.map((lineData, idx) => (<div
                                                                        key={`${market.marketName}-L${lineData.lineValue !== null ? lineData.lineValue : 'noLine'}-${idx}`}
                                                                        className="mb-2 p-3 bg-sleeper-surface-100 rounded-md border border-sleeper-border/30">
                                ...
                                {tabName === 'Game Bets' &&
                                    (processedTabsData.contentByTab[tabName] as ProcessedMarket[] || []).map((market, marketIdx) => (
                                        <div key={market.fullBetTypeName + '-' + marketIdx}
                                             className="p-4 bg-sleeper-surface-100 rounded-lg shadow-lg border border-sleeper-border">
                                            <h4 className="text-lg font-semibold text-sleeper-primary mb-4">{market.fullBetTypeName}</h4>
                                            {market.lines.map((lineData, lineIdx) => (<div
                                                key={`${market.fullBetTypeName}-L${lineData.lineValue !== null ? lineData.lineValue : 'noLine'}-${lineIdx}`}
                                                className="mb-3 last:mb-0 p-3 bg-sleeper-surface-200 rounded-md border border-sleeper-border/50">
                                                ...
                                                <div
                                                    className={`grid ${lineData.options.length === 3 ? 'grid-cols-3' : (lineData.options.length === 1 ? 'grid-cols-1' : 'grid-cols-2')} gap-3`}>
                                                    {lineData.options.sort((a, b) => {
                                                        ...
                                                        if (game && a.selection_name === game.home_team) return -1;
                                                        if (game && b.selection_name === game.home_team) return 1;
                                                        return a.selection_name.localeCompare(b.selection_name);
                                                    }).map(odd => renderOddButton(odd, odd.selection_name))}
                                                </div>
                                            </div>))}
                                        </div>
                                    ))}
