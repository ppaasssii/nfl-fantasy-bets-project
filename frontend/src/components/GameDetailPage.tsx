...
export interface AvailableBetDetail {
    ...
    odds: number; // American Odd from DB
    line?: number | null;
    is_active: boolean;
    ...
    // Fields populated by fetch-sports-data for player props
...
const GameDetailPage: React.FC = () => {
    ...
    const handleOddSelectWrapper = (odd: AvailableBetDetail) => {
        ...
        addToBetSlip(odd, false); // Pass false for isQuickBet from detail page
    };
    ...
    const processedTabsData = useMemo(() => {
        ...
        oddsData.forEach(odd => {
            ...
            if (marketKey.startsWith('player_')) dataByTab['Player Bets'].push(odd);
            else if (!periodKeysPattern.test(marketKey)) dataByTab['Game Bets'].push(odd);
        });
        ...
        if (dataByTab['Player Bets']?.length > 0) {
            const playerPropsByTeamAndName: {
                homePlayers: Record<string, Record<string, AvailableBetDetail[]>>;
                awayPlayers: Record<string, Record<string, AvailableBetDetail[]>>;
                unknownTeamPlayers: Record<string, Record<string, AvailableBetDetail[]>>;
            } = { homePlayers: {}, awayPlayers: {}, unknownTeamPlayers: {} };

            dataByTab['Player Bets'].forEach(odd => {
                const betTypeName = odd.bet_type?.name || "Unknown Prop Type";
                const playerName = odd.bet_subject_name || extractPlayerNameFromSelection(odd.selection_name, betTypeName, odd.bet_subject_name);
                const affiliation = odd.bet_subject_team_affiliation;

                let targetGroupForPlayerNameMapping;
                if (affiliation === 'home') targetGroupForPlayerNameMapping = playerPropsByTeamAndName.homePlayers;
                else if (affiliation === 'away') targetGroupForPlayerNameMapping = playerPropsByTeamAndName.awayPlayers;
                else targetGroupForPlayerNameMapping = playerPropsByTeamAndName.unknownTeamPlayers;

                if (!targetGroupForPlayerNameMapping[playerName]) targetGroupForPlayerNameMapping[playerName] = {};
                if (!targetGroupForPlayerNameMapping[playerName][betTypeName]) targetGroupForPlayerNameMapping[playerName][betTypeName] = [];
                targetGroupForPlayerNameMapping[playerName][betTypeName].push(odd);
            });

            contentByTab['Player Bets'] = {}; // This will hold the final processed structure for rendering
            ['homePlayers', 'awayPlayers', 'unknownTeamPlayers'].forEach(groupKey => {
                const playerGroup = playerPropsByTeamAndName[groupKey as keyof typeof playerPropsByTeamAndName];
                if (Object.keys(playerGroup).length > 0) {
                    contentByTab['Player Bets'][groupKey] = {}; // e.g., contentByTab['Player Bets']['homePlayers']
                    for(const playerName in playerGroup) {
                        const playerMarketsArray: ProcessedMarket[] = [];
                        for (const betTypeName in playerGroup[playerName]) {
                            playerMarketsArray.push(processOddsForMarketDisplay(playerGroup[playerName][betTypeName], betTypeName));
                        }
                        contentByTab['Player Bets'][groupKey][playerName] = playerMarketsArray;
                    }
                }
            });
        }
            ...
            {(!oddsData || processedTabsData.tabOrder.length === 0) && !loading ? (<p className="text-center text-lg text-gray-400 py-8">No active odds for this game.</p>) : (
                <Tab.Group>
                    ...
                    <Tab.Panels className="mt-2">
                        {processedTabsData.tabOrder.map((tabName) => (<Tab.Panel key={tabName} className="rounded-xl bg-transparent focus:outline-none space-y-4"> {/* Changed bg for panel */}
                            ...
                            {tabName === 'Player Bets' && ['homePlayers', 'awayPlayers', 'unknownPlayers'].map(playerGroupKey => {
                                ...
                                return (
                                    <Disclosure key={playerGroupKey} as="div" className="mb-4 bg-sleeper-surface-100 rounded-lg shadow-md" defaultOpen={playerGroupKey === 'homePlayers' || playerGroupKey === 'awayPlayers'}>
                                        {({open})=>(<>
                                            <Disclosure.Button className="flex justify-between w-full px-4 py-3 text-lg font-semibold text-left text-sleeper-primary hover:bg-sleeper-surface-200/80 rounded-lg"><span className="flex items-center">{React.createElement(icon, {className: "h-6 w-6 mr-3 text-sleeper-primary"})} {groupTitle}</span><ChevronUpIcon className={`${open?'rotate-180':''} w-5 h-5 text-sleeper-primary`}/></Disclosure.Button>
                                            <Disclosure.Panel className="px-2 py-3 text-sm border-t border-sleeper-border space-y-3">
                                                {Object.entries(playersToRender).map(([playerName, marketsForPlayer]) => (
                                                    <Disclosure key={`${playerName}-${playerGroupKey}`} as="div" className="bg-sleeper-surface-200 rounded-md shadow-sm" defaultOpen={false}>
                                                        {({open: playerOpen})=>(<>
                                                            <Disclosure.Button className="flex justify-between w-full px-3 py-2.5 text-md font-medium text-left text-sleeper-text-primary hover:bg-sleeper-border/20 rounded-t-md"><span className="flex items-center"><UserCircleIcon className="h-5 w-5 mr-2 text-sleeper-interactive"/>{playerName}</span><ChevronUpIcon className={`${playerOpen?'rotate-180':''} w-5 h-5 text-sleeper-interactive`}/></Disclosure.Button>
                                                            <Disclosure.Panel className="px-3 pt-2 pb-3 text-xs border-t border-sleeper-border/50 space-y-3">
                                                                {marketsForPlayer.map(market => (<div key={market.fullBetTypeName +'-'+ market.marketName} className="py-1">
                                                                    <h5 className="text-xs font-semibold text-sleeper-text-secondary mb-1.5 uppercase tracking-wider">{market.marketName}</h5>
                                                                    {market.lines.map((lineData,idx)=>(<div key={`${market.marketName}-L${lineData.lineValue !== null ? lineData.lineValue : 'noLine'}-${idx}`} className="mb-2 p-2.5 bg-sleeper-surface-100/70 rounded-md">
                            ...
                            {tabName === 'Game Bets' && (processedTabsData.contentByTab[tabName] as ProcessedMarket[] || []).map((market, marketIdx) => (
                                <div key={market.fullBetTypeName + marketIdx} className="p-4 bg-sleeper-surface-100 rounded-lg shadow-md border border-sleeper-border">
                                    ...
                                    {market.lines.map((lineData, lineIdx) => (<div key={`${market.fullBetTypeName}-L${lineData.lineValue !== null ? lineData.lineValue : 'noLine'}-${lineIdx}`} className="mb-2.5 last:mb-0 p-2.5 bg-sleeper-surface-200 rounded-md">
                                        {lineData.lineValue !== null && market.fullBetTypeName !== 'Moneyline' && !market.fullBetTypeName.toLowerCase().includes('even/odd') && !market.fullBetTypeName.toLowerCase().includes('3-way') && !market.fullBetTypeName.toLowerCase().includes('double chance') && (<p className="text-xs text-sleeper-text-secondary mb-1.5">Line: {lineData.lineValue > 0 ? `+${lineData.lineValue.toFixed(1)}`:lineData.lineValue.toFixed(1)}</p>)}
                                        <div className={`grid ${lineData.options.length === 3 ? 'grid-cols-3' : (lineData.options.length === 1 ? 'grid-cols-1' : 'grid-cols-2')} gap-2`}>
                                            {lineData.options.sort((a,b)=>{if(a.selection_name.toLowerCase().includes('over')||a.selection_name.toLowerCase().includes('yes'))return-1;if(b.selection_name.toLowerCase().includes('over')||b.selection_name.toLowerCase().includes('yes'))return 1; if(a.selection_name === game.home_team) return -1; if(b.selection_name === game.home_team) return 1; return a.selection_name.localeCompare(b.selection_name);}).map(odd=>renderOddButton(odd,odd.selection_name))}
                                        </div>
                                    </div>))}
                                </div>
                            ))}
